...
from musicstuffs import linkutils
...
class Music(commands.Cog):
    ...
    async def _play_song(self, ctx, *, track: str):
        ...
        if(await utils.is_connected(ctx) == None):
            if await audiocontroller.uconnect(ctx) == False:
                await ctx.send("DJ Party Parrot couldn't find the party")
                return
    ...
    @commands.command(name='repeat', description=config.HELP_LOOP_LONG, help=config.HELP_LOOP_SHORT, aliases=['l'])
    ...
    @commands.command(name='view', description=config.HELP_HISTORY_LONG, help=config.HELP_HISTORY_SHORT)
